# 1 "c:\\Users\\Dorian\\Documents\\cad\\zavrsni rad\\calc\\robot_arduino.cpp"
# 2 "c:\\Users\\Dorian\\Documents\\cad\\zavrsni rad\\calc\\robot_arduino.cpp" 2
 ...
 */
# 16 "c:\\Users\\Dorian\\Documents\\cad\\zavrsni rad\\calc\\robot_arduino.cpp"
int mode;
...
void leg_angle(int legNum)
{
    ...
    leg->l.x = ((leg->D.x)>0?(leg->D.x):-(leg->D.x)) - ((leg->s1.x)>0?(leg->s1.x):-(leg->s1.x));
    leg->l.z = leg->D.z + leg->s1.z;

    leg->D.y = tan(leg->motor1.angle) * leg->l.x - leg->s1.y;

    leg->s2.x = leg->s1.x + pow(-1, legNum) * 33.5 * cos(leg->motor1.angle);
    leg->s2.y = leg->s1.y + pow(-1, leg_pos) * 33.5 * sin(leg->motor1.angle);
    leg->s2.z = leg->s1.z;

    leg->L.x = ((leg->D.x)>0?(leg->D.x):-(leg->D.x)) - ((leg->s2.x)>0?(leg->s2.x):-(leg->s2.x));
    ...
    leg->motor2.angle = pow(-1, legNum) * (acos((pow(57.5, 2) + pow(leg->L.length(), 2) - pow(113, 2)) / (2 * 57.5 * leg->L.length())) - (P + R));
    leg->motor3.angle = pow(-1, legNum) * (3.1415926535897932384626433832795 - acos((pow(57.5, 2) + pow(113, 2) - pow(leg->L.length(), 2)) / (2 * 57.5 * 113)));
}
...
void legRotation(int j, bool backwards)
{
    ...
    int local_range;
    if (range == -1)
        local_range = ((local_leg->motor1.max)>0?(local_leg->motor1.max):-(local_leg->motor1.max)) + ((local_leg->motor1.min)>0?(local_leg->motor1.min):-(local_leg->motor1.min));
    else
        local_range = range;
    ...
    for (int j = 1; j <= 2;)
        for (int i = 1; i <= steps && j <= 2; i++)
        {
            ...
            for (int leg = 1; leg <= 6; leg++)
            {
                ...
                {
                    float offset = sin((float)i / steps * 3.1415926535897932384626433832795);
...
void sendMessage()
{
    if (!syncData && digitalRead(bluetooth) == 0x1) // if we havent synced and have connected to bluetooth
    {
        String buffer;
    ...
    if (digitalRead(bluetooth) == 0x0) // if bluetooth device is disconnected reset sync data
        syncData = false;
}
...
# 405 "c:\\Users\\Dorian\\Documents\\cad\\zavrsni rad\\calc\\robot_arduino.cpp"
    case 0:
        // wake up
        Serial.print("leg1");
        Serial.print("leg2");
...
# 419 "c:\\Users\\Dorian\\Documents\\cad\\zavrsni rad\\calc\\robot_arduino.cpp"
        break;
    case 1:
        // walk
        move();
